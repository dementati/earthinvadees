...
class FighterAIController(object):
	...
	def align_to(self, dt, v):
		...
		m = math.fabs(a)/180
		if a < 0:
			self.ship.turn_left(dt, m)
		else:
			self.ship.turn_right(dt, m)
	...
	def stop(self, dt):
		if self.ship.velocity.get_magnitude() > 0.1:
			a = self.align_to(dt, -self.ship.velocity)
			if math.fabs(a) < 10:
				self.ship.accelerate(dt)
...
class TerranMothershipAIController(FighterAIController):
	def stop(self, dt):
		if self.ship.velocity.get_magnitude() > 0.1:
